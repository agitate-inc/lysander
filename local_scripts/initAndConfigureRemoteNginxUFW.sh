...
scp -i ~/.ssh/rmroot_rsa ~/lysander/resources/scp_scripts/installNginxAndSetupUFW.scp root@"$remoteIP":/root;
...
ssh -i ~/.ssh/rm_root_rsa root@"$remoteIP" "chgrp -R letsencrypt /etc/letsencrypt chmod -R g=rX /etc/letsencrypt"
