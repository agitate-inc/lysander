...
scp -i ~/.ssh/rmroot_rsa ~/lysander/resources/scp_scripts/makeManager.scp root@"$remoteIP":/root;
ssh -i ~/.ssh/rmroot_rsa root@"$remoteIP" "bash ~/makeManager.scp";
...
ssh-keygen -t rsa -f ~/.ssh/rmmanager_rsa -N '';
scp -i ~/.ssh/rmroot_rsa ~/.ssh/rmmanager_rsa.pub root@"$remoteIP":/root;
