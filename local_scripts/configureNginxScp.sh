...
sed -i "6s/.*/server_name "$siteName";/" ~/lysander/resources/remote.nginx_precert.scp;
sed -i "25s|.*|ssl_certificate /etc/letsencrypt/live/"$siteName"/fullchain.pem;|" ~/lysander/resources/remote.nginx_postcert.scp
sed -i "26s|.*|ssl_certificate /etc/letsencrypt/live/"$siteName"/privkey.pem;|" ~/lysander/resources/remote.nginx_postcert.scp
