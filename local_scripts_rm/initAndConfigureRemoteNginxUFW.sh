...
##TESTING : DRY RUN ACTIVE ON LETSENCRYPT.
...
ssh -i ~/.ssh/rmroot_rsa root@"$remoteIP" "sudo certbot certonly -d "$siteName" -q --nginx --keep-until-expiring --dry-run --register-unsafely-without-email --agree-tos"
